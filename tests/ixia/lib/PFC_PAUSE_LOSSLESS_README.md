...
- Test data traffic: Data packets sent from the IXIA tx port to the IXIA rx port at the priority (e.g., 3) that we want to test. Note that the packets should be marked with the correct DSCP value (e.g., DSCP 3 for priority 3). The traffic demand should be 50% of the line rate.

- Background data traffic: Data packets sent from the IXIA tx port to the IXIA rx port at the other priorities that we want to test. The traffic demand should be 50% of the line rate.

- PFC pause storm: Persistent PFC pause frames from the IXIA rx port to the IXIA tx port. The pause frames should only pause the priority of test data traffic. The pause frames should be sent fast enough to fully block the switch port, e.g., sending interval is smaller than pause duration.

We should start the PFC pause storm first to fully block the test priority at the switch. Then we can start test data traffic and background data traffic simultaneously. Test data traffic and background data traffic should last for a fixed duration (e.g., 5 seconds). PFC pause storm can only stop after both data traffic items stop. After both data traffic items stop, we check if the IXIA rx port receives (1) all the sent frames of background data traffic (which should not be impacted by PFC), and (2) zero frame of test data traffic (which should be fully blocked by PFC pause storm).
